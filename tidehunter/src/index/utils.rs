...
/// - If both sources have no entries, return None
/// - If only one source has an entry, return it (if valid)
...
///
/// Returns None if no valid entries were found, or Some((key, val)) for the next entry.
pub fn take_next_entry(
    in_memory: Option<(Bytes, WalPosition)>,
    on_disk: Option<(Bytes, WalPosition)>,
    direction: Direction,
) -> Option<Result<(Bytes, WalPosition), Bytes>> {
...
mod tests {
    ...
    fn test_take_next_entry() {
        ...
        assert_eq!(result, Some(Ok((key3.clone(), valid_pos))));
        ...
        assert_eq!(result, Some(Err(key3.clone())));
