...
impl BenchmarkMetrics {
    pub fn new_in(registry: &Registry, config: &StressTestConfigs) -> Arc<Self> {
        ...
        this.param_read_percentage.set(config.stress_client_parameters.read_percentage as i64);
        this.param_zipf_exponent.set(config.stress_client_parameters.zipf_exponent);
        this.param_write_threads.set(config.stress_client_parameters.write_threads as i64);
        this.param_mixed_threads.set(config.stress_client_parameters.mixed_threads as i64);
        this.param_write_size.set(config.stress_client_parameters.write_size as i64);
        this.param_key_len.set(config.stress_client_parameters.key_len as i64);
        this.param_operations.set(config.stress_client_parameters.operations as i64);
        this.param_writes.set(config.stress_client_parameters.writes as i64);
        this.param_background_writes.set(config.stress_client_parameters.background_writes as i64);
        this.param_direct_io.set(if config.db_parameters.direct_io { 1 } else { 0 });
        this.param_num_flusher_threads.set(config.db_parameters.num_flusher_threads as i64);
        this.param_frag_size.set(config.db_parameters.frag_size as i64);
        this.param_max_maps.set(config.db_parameters.max_maps as i64);
        this.param_max_dirty_keys.set(config.db_parameters.max_dirty_keys as i64);
